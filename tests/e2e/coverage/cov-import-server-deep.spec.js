...
test('Import server deep: connect to local CaSS endpoint and exercise search flow', async ({
  page
}) => {
  let uid;
  ...
  await frameworksOrError.first().waitFor({
    state: 'visible'
  }).catch(() => {});
  ...
  if (await frameworkSelect.isVisible().catch(() => false)) {
    ...
    if (await firstOption.isVisible().catch(() => false)) {
      await frameworkSelect.selectOption({
        index: 0
      });
    }
  ...
  if (await caseConnectBtn.isVisible().catch(() => false)) {
    ...
    await errorModal.first().waitFor({
      state: 'visible'
    }).catch(() => {});
  }
});