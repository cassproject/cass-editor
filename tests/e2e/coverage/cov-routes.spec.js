...
test('Route: welcome page loads', async ({
  page
}) => {
...
test('Route: frameworks list', async ({
  page
}) => {
...
test('Route: crosswalk page', async ({
  page
}) => {
...
test('Route: configuration page', async ({
  page
}) => {
  let uid;
...
test('Route: legacy login page', async ({
  page
}) => {
...
test('SideNav: navigation links visible', async ({
  page
}) => {
  await page.goto('/#/frameworks?server=http://localhost/api/');
  await page.waitForLoadState('domcontentloaded');
  await expect(page.locator('#app-side-nav-bar')).toBeVisible();
...
test('SideNav: navigate between sections', async ({
  page
}) => {
  let uid;
  uid = await loginAndNavigate(page, uid);
...
test('SideNav: logged-in state shows user icon', async ({
  page
}) => {
  let uid;
  uid = await loginAndNavigate(page, uid);
