...
test('Search: frameworks search bar type and clear', async ({
  page
}) => {
...
test('Search: search bar filter exercises store commit', async ({
  page
}) => {
  await page.goto('/#/frameworks?server=http://localhost/api/');
  await expect(page.locator('#frameworks')).toBeVisible();
...
test('FilterAndSort: toggle owned-by-me filter', async ({
  page
}) => {
  await page.goto('/#/frameworks?server=http://localhost/api/');
  await expect(page.locator('#frameworks')).toBeVisible();
...
test('FilterAndSort: exercise sort options', async ({
  page
}) => {
  await page.goto('/#/frameworks?server=http://localhost/api/');
  await expect(page.locator('#frameworks')).toBeVisible();
  ...
  const result = await page.evaluate(() => {
    const filterEl = document.querySelector('.filter-and-sort') || document.querySelector('.cass--list--filter') || document.querySelector('#frameworks');
    if (!filterEl) return {
      noFilterEl: true
    };
    ...
    return {
      noFilterAndSort: true
    };
  });
  expect(result).toBeDefined();
});
test('List: framework list exercises List.vue', async ({
  page
}) => {
  await page.goto('/#/frameworks?server=http://localhost/api/');
  await expect(page.locator('#frameworks')).toBeVisible();
...
test('List: scroll and load more items', async ({
  page
}) => {
  await page.goto('/#/frameworks?server=http://localhost/api/');
  await expect(page.locator('#frameworks')).toBeVisible();
