...
test.describe('NarrowSideNav Coverage', () => {
  let uid;
  test('NarrowSideNav renders when sidebar is collapsed (not logged in)', async ({
    page
  }) => {
  ...
  test('NarrowSideNav shows all navigation sections', async ({
    page
  }) => {
    await page.goto('/#/frameworks?server=http://localhost/api/');
    await page.waitForLoadState('domcontentloaded');
  ...
  test('NarrowSideNav after login shows user info and users link', async ({
    page
  }) => {
      ...
      const navData = await page.evaluate(() => {
        ...
        if (el && el.__vueParentComponent) {
          ...
          return {
            displayName: vm.displayName,
            isLoggedOn: vm.isLoggedOn,
            loggedOnPerson: vm.loggedOnPerson ? {
              name: vm.loggedOnPerson.name,
              email: vm.loggedOnPerson.email
            } : null
          };
        }
        return null;
  ...
  test('NarrowSideNav navigation link clicks', async ({
    page
  }) => {
    await page.goto('/#/frameworks?server=http://localhost/api/');
    await page.waitForLoadState('domcontentloaded');
