...
test('Thing: framework Thing rendering', async ({
  page
}) => {
...
test('Breadcrumbs: navigate framework and check breadcrumbs', async ({
  page
}) => {
  await page.goto('/#/frameworks?server=http://localhost/api/');
  await expect(page.locator('#frameworks')).toBeVisible();
  ...
  const result = await page.evaluate(() => {
    const breadcrumbEl = document.querySelector('.lode__breadcrumbs') || document.querySelector('.breadcrumbs') || document.querySelector('[class*="breadcrumb"]');
    ...
    return {
      hasElement: true,
      text: breadcrumbEl.textContent.substring(0, 100)
    };
  });
  expect(result).toBeDefined();
});
test('ListItemInfo: hover or click to show info', async ({
  page
}) => {
  await page.goto('/#/frameworks?server=http://localhost/api/');
  await expect(page.locator('#frameworks')).toBeVisible();
  ...
  const result = await page.evaluate(() => {
    const infoEl = document.querySelector('.list-item-info') || document.querySelector('.lode__list-item-info') || document.querySelector('[class*="ListItemInfo"]');
    if (!infoEl) {
...
test('Thing: double-click navigates into framework from list', async ({
  page
}) => {
  await page.goto('/#/frameworks?server=http://localhost/api/');
  await expect(page.locator('#frameworks')).toBeVisible();
  ...
  await thingItems.first().waitFor({
    state: 'visible'
  });
  ...
  const viewResult = await page.evaluate(() => {
    ...
    return {
      selectedFramework: store.state.editor?.framework?.shortId?.() != null || store.state.editor?.framework != null,
