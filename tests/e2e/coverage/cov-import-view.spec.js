...
test.describe('Import View Coverage', () => {
  test('Exercise Import page load and computed properties', async ({
    page
  }) => {
  ...
  test('Exercise Import Vue component methods', async ({
    page
  }) => {
    await page.goto('/#/import?server=http://localhost/api/');
    await page.waitForLoadState('domcontentloaded');
  ...
  test('Exercise import page with different import types', async ({
    page
  }) => {
    ...
    const result = {};
    try {
      if (await fileTab.first().isVisible()) {
    ...
    try {
      if (await urlTab.first().isVisible()) {
