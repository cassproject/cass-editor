...
test('Property: click competency and interact with text field', async ({
  page
}) => {
  let uid;
...
test('Property: inspect all property computed values', async ({
  page
}) => {
  await page.goto('/#/frameworks?server=http://localhost/api/');
...
test('Property: secondary view exposes more properties', async ({
  page
}) => {
  await page.goto('/#/frameworks?server=http://localhost/api/');
  await expect(page.locator('#frameworks')).toBeVisible();
...
test('Property: PropertyString rendering', async ({
  page
}) => {
  let uid;
  uid = await loginAndNavigate(page, uid);
