...
test('CA-152: Property editing adheres to allowed type', async ({
  page
}) => {
    ...
    for (const el of propertyEls) {
      ...
      if (vm && vm.schema) {
        ...
        props.push({
          ...
          rangeType: rangeStr.includes('langString') ? 'langString' : rangeStr.includes('URL') || rangeStr.includes('anyURI') ? 'URL' : rangeStr.includes('Concept') ? 'concept' : 'other',
