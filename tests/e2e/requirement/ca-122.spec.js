...
test('CA-122: Property editing adheres to allowed type', async ({
  page
}) => {
    ...
    for (const el of propertyEls) {
      ...
      if (vm && vm.schema) {
        ...
        properties.push({
          ...
          rangeType: rangeStr.includes('langString') ? 'langString' : rangeStr.includes('URL') || rangeStr.includes('anyURI') ? 'URL' : rangeStr.includes('Concept') ? 'concept' : rangeStr.includes('Competency') ? 'object' : 'other',
          isText: isText
        });
      }
    }
