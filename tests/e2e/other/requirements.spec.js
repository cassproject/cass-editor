...
test.describe('Requirements Verification', () => {
  ...
  test('CA-83: Configuration Editor accessibility', async ({
    page
  }) => {
  ...
  test('CA-116: Frameworks list accessibility', async ({
    ...
    await expect(page.locator('h2', {
      hasText: 'Available Items'
    })).toBeVisible();
  });

  // CA-223: CaSS Editor shall allow an individual to create a user.
  test('CA-223: Create Account accessibility', async ({
    page
  }) => {
    await page.goto('/#/createAccount?server=http://localhost/api/');
    await expect(page.locator('h3.modal-card-title', {
      hasText: 'Create CaSS Account'
    })).toBeVisible();
  ...
  test('CA-134: Framework search filtering', async ({
    page
  }) => {
    await page.goto('/#/frameworks?server=http://localhost/api/');
  ...
  test('CA-224: User creation fields', async ({
    page
  }) => {
  ...
  test('CA-131: Sort by last modified', async ({
    page
  }) => {
    ...
    await expect(page.locator('.filter-sort-header').filter({
      hasText: 'Sort by'
    })).toBeVisible();
  ...
  test('CA-245: Published status indication', async ({
    page
  }) => {
    await page.goto('/#/frameworks?server=http://localhost/api/');
