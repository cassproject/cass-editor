...
export default {
    ...
    methods: {
        removeObject: function (thing) {
            ...
            var me = this;
            var thisFramework = store.editor().framework;
            ...
            thisFramework.removeCompetency(thing.shortId(), async function () {
                ...
                store.editor().addEditsToUndo([{ operation: "update", id: framework.shortId(), fieldChanged: ["competency", "relation", "level"], initialValue: [initialCompetencies, initialRelations, initialLevels] }]);
                if (store.editor().private === true && EcEncryptedValue.encryptOnSaveMap[f.id] !== true) {
                    framework = await EcEncryptedValue.toEncryptedValue(framework);
                }
                window.repo.saveTo(framework, function () {
                    store.editor().setFramework(thisFramework);
                }, console.error);
            }, console.log);
        },
        deleteObject: function (thing) {
            ...
            if (thing.shortId() === framework.shortId()) {
                ...
                store.editor().setSelectedCompetency(thing);
