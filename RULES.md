...
- **Use browser when creating tests**.When creating tests, use the browser recording feature to record a pathway through the application, then put it in the correct folder.
- **Automatic Test Harness**. A test harness is running in the background and will rerun tests automatically. Do not manually run tests or delete the Playwright output file. Instead, check the modified date of `playwright-output.txt` or files in test-results and read them get the latest test results.
- **Automatic Build**. A build is running in the background and will build the project automatically. Do not manually build the project or delete the build output file.
- **Console Log Capture**. When a Playwright test fails, the test suite is configured to automatically capture everything the web application wrote to the browser's console up to the point of failure. These logs are saved as a `browser-console-logs.txt` artifact in the failure's specific `test-results/` folder. This is a crucial debugging tool when a test fails.